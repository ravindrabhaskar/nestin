...
describe('booking lifecycle (tenant → owner CRM → inventory → payments)', () => {
  ...
  test('full lifecycle: book → double-booking blocked → staff approves → bed occupied → customer & payment created → tenant sees it', async () => {
    ...
    const bedAfter = approved.data.property.rooms
      .find((r: any) => r.id === room.id)
      .beds.find((b: any) => b.id === bed.id);
    ...
    const moveOut = await api.post(
      `/crm/customers/${approved.data.customer.id}/move-out`,
      { moveOutDate: '2027-01-31', reason: 'Relocation' },
      owner
    );
    assert.equal(moveOut.status, 200);
    assert.equal(moveOut.data.customer.tenantStatus, 'Inactive');
    const bedReleased = moveOut.data.property.rooms
      .find((r: any) => r.id === room.id)
      .beds.find((b: any) => b.id === bed.id);
  ...
  test('tenant can cancel their own pending booking and it releases nothing that was not reserved', async () => {
    ...
    const other = await api.post('/auth/register', {
      email: 'third.tenant@example.com',
      password: 'Strong123',
      fullName: 'Third Tenant',
    });
    assert.equal(
      (await api.post(`/tenant/bookings/${booking.data.booking.id}/cancel`, {}, other.data.token)).status,
      404,
      'another tenant cannot cancel it'
    );
    const cancelled = await api.post(
      `/tenant/bookings/${booking.data.booking.id}/cancel`,
  ...
  test("owners cannot touch another owner's bookings or properties", async () => {
    const marketplaceProp = (await api.get('/properties/public')).data.find(
      (p: any) => p.ownerId === 'owner-marketplace'
    );
    assert.ok(marketplaceProp);
    const update = await api.put(`/properties/owner/${marketplaceProp.id}`, { name: 'Hijacked' }, owner);
    assert.equal(update.status, 404);
    const bed = await api.patch(
      `/properties/owner/${marketplaceProp.id}/beds`,
      { roomId: 'x', bedId: 'y', isOccupied: true },
      owner
    );
