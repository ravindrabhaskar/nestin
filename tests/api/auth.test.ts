...
describe("auth & authorization", () => {
  ...
  test("registration enforces password strength and cannot create privileged roles", async () => {
    const weak = await api.post("/auth/register", { email: "weak@example.com", password: "short", fullName: "Weak Pass" });
    assert.equal(weak.status, 400);

    const escalate = await api.post("/auth/register", { email: "admin-wannabe@example.com", password: "Strong123", fullName: "Wannabe", role: "super_admin" });
    assert.equal(escalate.status, 400, "unknown role values are rejected");

    const ok = await api.post("/auth/register", { email: "new.user@example.com", password: "Strong123", fullName: "New User", role: "tenant" });
    assert.equal(ok.status, 201);
    assert.equal(ok.data.user.role, "tenant");

    const dup = await api.post("/auth/register", { email: "new.user@example.com", password: "Strong123", fullName: "Again" });
  ...
  test("forged, tampered and unsigned tokens are rejected", async () => {
    ...
    assert.equal((await api.get("/auth/me", `${header}.${payload}.sig-legacy`)).status, 401, "legacy sig- prefix is not accepted");

    const forgedPayload = Buffer.from(JSON.stringify({ ...JSON.parse(Buffer.from(payload, "base64url").toString()), role: "super_admin" })).toString("base64url");
    assert.equal((await api.get("/auth/me", `${header}.${forgedPayload}.${token.split(".")[2]}`)).status, 401, "payload tampering breaks the signature");
  ...
  test("profile updates cannot change role, email or owner scope", async () => {
    const token = await login(api, DEMO.tenant);
    const res = await api.put("/auth/profile", { role: "super_admin", email: "hijack@example.com", ownerId: "owner-001", city: "Pune" }, token);
    assert.equal(res.status, 200);
  ...
  test("changing the password signs out other devices", async () => {
    ...
    const res = await api.post("/auth/change-password", { currentPassword: DEMO.password, newPassword: "Rotated123" }, laptop);
    assert.equal(res.status, 200);
  ...
  test("role boundaries: tenant → CRM 403, owner → admin 403, staff without permission 403", async () => {
    ...
    assert.equal((await api.post("/rbac/roles", { name: "Escalate", permissions: { "roles.manage": true } }, staff)).status, 403);
  });
  ...
  test("login attempts are rate limited", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      results.push((await api.post("/auth/login", { email: "bruteforce@example.com", password: `guess-${i}-xx` })).status);
    }
