...
describe('property publishing workflow, RBAC and admin', () => {
  ...
  test('owner creates a draft, cannot self-publish, submits for review, admin approves, listing goes live', async () => {
    const created = await api.post(
      '/properties/owner',
      { name: 'Test Residency', isNestinVerified: true, status: 'published', rooms: [] },
      owner
    );
    ...
    assert.equal(
      (await api.put(`/properties/owner/${id}`, { status: 'published' }, owner)).status,
      409,
      'owner cannot flip status to published'
    );
    ...
    const complete = await api.put(
      `/properties/owner/${id}`,
      {
        ...
        rooms: [
          {
            ...
            maintenance: 500,
            beds: [
              { id: 'b1', bedNumber: 'A', isOccupied: false },
              { id: 'b2', bedNumber: 'B', isOccupied: false },
            ],
          },
        ],
        pricing: {
          minRent: 12000,
          securityDepositRefundPolicy: 'Refundable',
          electricity: { type: 'Metered', label: 'Metered' },
          water: { type: 'Included', label: 'Included' },
          foodMess: { type: 'Included', label: 'Included' },
          laundryAndHousekeeping: { type: 'Included', label: 'Included' },
          maintenance: { type: 'Included', amount: 500, label: 'Included' },
          bookingFee: 999,
        },
        amenities: [1, 2, 3].map((i) => ({
          id: `a${i}`,
          name: `Amenity ${i}`,
          category: 'Comfort',
          isAvailable: true,
        ...
        policies: {
          curfew: '11 PM',
          visitorPolicy: 'Lounge only',
          smokingAndAlcohol: 'No',
          cancellationPolicy: '30 days',
        ...
        location: {
          ...
          latitude: 17.4,
          longitude: 78.3,
          formattedAddress: '1 Test Road, Madhapur',
        },
        caretaker: {
          name: 'Care Taker',
    ...
    assert.equal(
      (await api.get(`/properties/public/${complete.data.slug}`)).status,
      404,
  ...
  test('RBAC: staff accounts are created with a login, roles are owner-scoped, owner role is locked', async () => {
    ...
    assert.equal(
      (await api.put(`/rbac/roles/${ownerRole.id}`, { ...ownerRole, permissions: { 'dashboard.view': false } }, owner))
        .status,
      409
    );
    ...
    const role = await api.post(
      '/rbac/roles',
      {
        ...
        authorityLevel: 'low',
        permissions: { 'visitors.view': true, 'visitors.create': true, 'bogus.permission': true },
      },
      owner
    );
    ...
    const emp = await api.post(
      '/rbac/employees',
      {
        name: 'Nisha Warden',
        email: 'nisha.warden@example.com',
    ...
    const staffLogin = await api.post('/auth/login', {
      email: 'nisha.warden@example.com',
      password: emp.data.temporaryPassword,
    });
    ...
    assert.equal(
      (await api.post('/crm/leads', { fullName: 'X', phone: '+91 90000 00000' }, staffLogin.data.token)).status,
      403
    );
    assert.equal(
      (
        await api.post(
          '/crm/visitors',
          { visitorName: 'V', phone: '+91 90000 00000', propertyId: 'prop-banyan-premium', visitDate: '2026-10-10' },
          staffLogin.data.token
        )
      ).status,
      201
    );
    ...
    const withOverride = await api.put(
      `/rbac/employees/${emp.data.id}`,
      { overrides: { 'leads.create': true } },
      owner
    );
    assert.equal(withOverride.status, 200);
    assert.equal(
      (await api.post('/crm/leads', { fullName: 'Override Lead', phone: '+91 90000 00002' }, staffLogin.data.token))
        .status,
      201
    );
  ...
  test('CRM documents are owner-scoped and validated', async () => {
    ...
    const hijack = await api.put(
      `/crm/leads/${lead.data.id}`,
      { ...lead.data, ownerId: 'owner-marketplace', stage: 'Converted' },
      owner
    );
    ...
    assert.equal(
      (await api.put(`/crm/leads/${lead.data.id}`, { stage: 'Lost' }, marketplaceOwner)).status,
      404,
  ...
  test('public forms land in the admin inbox and admin can manage accounts', async () => {
    const contact = await api.post('/public/contact', {
      fullName: 'Site Visitor',
