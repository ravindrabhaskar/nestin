...
export const RBACProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const persistRole = (role: Role, previous: Role | null, label: string) => {
    ApiClient.rbac
      .upsertRole(role as unknown as { id?: string } & Record<string, unknown>)
  ...
  const persistEmployee = (employee: Employee, previous: Employee | null, label: string) => {
    ApiClient.rbac
      .upsertEmployee(employee as unknown as { id?: string } & Record<string, unknown>)
      .then((saved: Employee & { temporaryPassword?: string }) => {
        ...
        if (temporaryPassword) {
          syncBus.publish(
            'success',
            `Login created for ${rest.name} (${rest.email}). Temporary password: ${temporaryPassword}`
          );
        }
      })
      .catch((err) => {
  ...
  const findSelf = useCallback(
    () => employees.find((e) => e.id === user?.employeeId) || employees.find((e) => e.email === user?.email),
    [employees, user?.email, user?.employeeId]
  );
  ...
  const hasPermission = useCallback(
    (permissionId: string, employeeId?: string, propertyId?: string): boolean => {
      ...
      if (
        propertyId &&
        targetEmp.assignedProperties &&
        !targetEmp.assignedProperties.includes('all') &&
        !targetEmp.assignedProperties.includes(propertyId)
      )
        return false;
  ...
  const toggleRolePermission = useCallback(
    (roleId: string, permissionId: string): boolean => {
      ...
      const updated: Role = {
        ...targetRole,
        permissions: { ...targetRole.permissions, [permissionId]: newValue },
  ...
  const batchUpdateRolePermissions = useCallback(
    (roleId: string, newPermissions: Record<string, boolean>) => {
      ...
      const updated: Role = {
        ...targetRole,
        permissions: { ...targetRole.permissions, ...newPermissions },
        updatedAt: new Date().toISOString(),
      };
      setRoles((prev) => replaceById(prev, updated));
  ...
  const updateRole = useCallback(
    (
      ...
      const updated: Role = {
        ...
        name: roleData.name,
        description: roleData.description,
        authorityLevel: roleData.authorityLevel || targetRole.authorityLevel,
        color: roleData.color || targetRole.color,
        permissions: targetRole.isOwnerRole ? getAllPermissionsTrue() : roleData.permissions || targetRole.permissions,
        updatedAt: new Date().toISOString(),
      };
      setRoles((prev) => replaceById(prev, updated));
  ...
  const duplicateRole = useCallback(
    (roleId: string, newName?: string): Role => {
      ...
      const clonedRole: Role = {
        id: uid('role'),
        name: newName || `${sourceRole?.name || 'Role'} (Copy)`,
  ...
  const updateEmployee = useCallback(
    (employeeId: string, updates: Partial<Employee>) => {
      ...
      logAction({
        ...
        targetType: 'employee',
        targetId: employeeId,
        targetName: updates.name || res.current.name,
        action: 'Employee Profile Updated',
        note: `Updated employee details for ${updates.name || res.current.name}.`,
      });
    },
  ...
  const changeEmployeeRole = useCallback(
    (employeeId: string, newRoleId: string): boolean => {
      ...
      const res = mutateEmployee(
        employeeId,
        (e) => ({ ...e, roleId: newRoleId, roleName: newRole.name }),
      ...
      logAction({
        ...
        targetType: 'employee',
        targetId: employeeId,
        targetName: res.current.name,
        action: 'Role Changed',
        oldValue: res.current.roleName,
        newValue: newRole.name,
        note: `Changed role for ${res.current.name} from "${res.current.roleName}" to "${newRole.name}".`,
      });
      return true;
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const toggleEmployeeStatus = useCallback(
    (employeeId: string) => {
      const res = mutateEmployee(
        employeeId,
        (e) => ({ ...e, status: e.status === 'active' ? 'inactive' : 'active' }),
        'Could not change the employee status'
      );
      if (!res) return;
      logAction({
        actorId: currentActor.id,
        ...
        targetId: employeeId,
        targetName: res.current.name,
        action: res.updated.status === 'active' ? 'Employee Activated' : 'Employee Deactivated',
        oldValue: res.current.status,
        newValue: res.updated.status,
        note: `${currentActor.name} marked ${res.current.name} as ${res.updated.status}.`,
      });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [currentActor, logAction]
  ...
  const setEmployeeOverride = useCallback(
    (employeeId: string, permissionId: string, overrideValue: boolean | null) => {
      ...
      logAction({
        ...
        targetId: employeeId,
        targetName: `${res.current.name} (${res.current.roleName})`,
        action: overrideValue === null ? 'Override Cleared' : overrideValue ? 'Override Granted' : 'Override Denied',
        permissionId,
        permissionLabel: permDef?.label || permissionId,
  ...
  const resetEmployeePassword = useCallback(
    async (employeeId: string) => {
      ...
      logAction({
        ...
        note: `Temporary password issued for ${emp?.name || employeeId}.`,
      });
    ...
    (employeeIds: string[], roleId: string) => {
      ...
      for (const id of employeeIds)
        mutateEmployee(
          id,
          (e) =>
            rolesRef.current.find((r) => r.id === e.roleId)?.isOwnerRole ? e : { ...e, roleId, roleName: role.name },
    ...
    (employeeIds: string[], propertyIds: string[]) => {
      for (const id of employeeIds)
        mutateEmployee(
          ...
          'Could not update property access'
        );
      logAction({
        actorId: currentActor.id,
        actorName: currentActor.name,
  ...
  const bulkSetStatus = useCallback(
    (employeeIds: string[], status: 'active' | 'inactive') => {
      for (const id of employeeIds)
        mutateEmployee(
          id,
          (e) => (rolesRef.current.find((r) => r.id === e.roleId)?.isOwnerRole ? e : { ...e, status }),
