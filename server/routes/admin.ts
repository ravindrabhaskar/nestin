...
adminRouter.get(
  '/stats',
  wrap((_req, res) => sendOk(res, admin.stats()))
);
adminRouter.get(
  '/users',
  wrap((req, res) =>
    sendOk(res, admin.listUsers(typeof req.query.role === 'string' ? { role: req.query.role as never } : {}))
  )
);
adminRouter.put(
  '/users/:id/status',
  wrap((req, res) =>
    sendOk(
      ...
      admin.setUserStatus(
        currentUser(req),
        req.params.id,
        req.body?.status === 'suspended' ? 'suspended' : 'active',
        ctx(req)
      )
    )
  )
...
adminRouter.get(
  '/properties',
  wrap((_req, res) => sendOk(res, props.listAll()))
);
adminRouter.post(
  '/properties/:id/approve',
  wrap((req, res) => sendOk(res, props.adminApprove(currentUser(req), req.params.id, req.body || {}, ctx(req))))
);
adminRouter.post(
...
adminRouter.patch(
  ...
  wrap((req, res) => sendOk(res, props.adminSetBadges(currentUser(req), req.params.id, req.body || {}, ctx(req))))
);

adminRouter.get(
  '/bookings',
  wrap((_req, res) => sendOk(res, admin.listAllBookings()))
);
adminRouter.get(
  '/inbound',
  wrap((req, res) =>
    sendOk(res, admin.listInbound(typeof req.query.kind === 'string' ? (req.query.kind as never) : undefined))
  )
);
adminRouter.put(
  '/inbound/:id',
  wrap((req, res) => sendOk(res, admin.updateInbound(currentUser(req), req.params.id, req.body?.status, ctx(req))))
);
adminRouter.get(
  '/audit',
  wrap((req, res) =>
...
adminRouter.get(
  '/support',
  wrap((_req, res) => sendOk(res, support.listAll()))
);
adminRouter.post(
  '/support/:id/messages',
  wrap((req, res) =>
    sendOk(res, support.replyAsSupport(currentUser(req), req.params.id, req.body || {}, ctx(req), { admin: true }))
  )
);
adminRouter.post(
  '/support/:id/resolve',
