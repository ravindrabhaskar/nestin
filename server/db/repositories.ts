...
import type { LeadItem, BookingItem, VisitorItem, CustomerItem, CRMActivityLog, OwnerCRMNotification } from "../../src/types/crm";
...
export const users = {
  ...
  findByEmail(emailAddress: string): UserRecord | null {
    return rowToUser(getDb().prepare("SELECT * FROM users WHERE email = ?").get(emailAddress.toLowerCase()) as unknown as UserRow | undefined);
  },
  insert(user: Omit<UserRecord, "createdAt" | "updatedAt">): UserRecord {
    ...
    getDb()
      ...
      .run(user.id, user.email.toLowerCase(), user.passwordHash, user.role, user.ownerId, user.fullName, user.status, user.authProvider, JSON.stringify(user.data), ts, ts);
    return { ...user, createdAt: ts, updatedAt: ts };
  },
  update(id: string, patch: Partial<Pick<UserRecord, "passwordHash" | "role" | "ownerId" | "fullName" | "status" | "authProvider" | "data" | "email">>): UserRecord | null {
    ...
    getDb()
      ...
      .run(merged.email.toLowerCase(), merged.passwordHash, merged.role, merged.ownerId, merged.fullName, merged.status, merged.authProvider, JSON.stringify(merged.data), merged.updatedAt, id);
    return merged;
  },
  list(filter: { role?: AppRole; ownerId?: string } = {}, limit = 500): UserRecord[] {
    ...
    if (filter.ownerId) { clauses.push("owner_id = ?"); params.push(filter.ownerId); }
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    const rows = getDb().prepare(`SELECT * FROM users ${where} ORDER BY created_at DESC LIMIT ${Math.floor(limit)}`).all(...params) as unknown as UserRow[];
...
interface SessionRow {
  id: string; user_id: string; token_hash: string; device: string; browser: string; ip: string; location: string;
  created_at: string; last_active_at: string; expires_at: string; revoked_at: string | null;
}

const rowToSession = (r: SessionRow | undefined): SessionRecord | null =>
  r
    ? { id: r.id, userId: r.user_id, tokenHash: r.token_hash, device: r.device, browser: r.browser, ip: r.ip, location: r.location, createdAt: r.created_at, lastActiveAt: r.last_active_at, expiresAt: r.expires_at, revokedAt: r.revoked_at }
    : null;
...
export const sessions = {
  insert(s: Omit<SessionRecord, "createdAt" | "lastActiveAt" | "revokedAt">): SessionRecord {
    const ts = nowIso();
    getDb()
      .prepare(`INSERT INTO sessions (id, user_id, token_hash, device, browser, ip, location, created_at, last_active_at, expires_at, revoked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`)
  ...
  get(id: string): SessionRecord | null {
    return rowToSession(getDb().prepare("SELECT * FROM sessions WHERE id = ?").get(id) as unknown as SessionRow | undefined);
  },
  ...
  revokeAllForUser(userId: string, exceptId?: string): void {
    getDb().prepare("UPDATE sessions SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL AND id != ?").run(nowIso(), userId, exceptId || "");
  },
  listActiveForUser(userId: string): SessionRecord[] {
    const rows = getDb()
      .prepare("SELECT * FROM sessions WHERE user_id = ? AND revoked_at IS NULL AND expires_at > ? ORDER BY last_active_at DESC")
      .all(userId, nowIso()) as unknown as SessionRow[];
    return rows.map((r) => rowToSession(r) as SessionRecord);
  },
  purgeExpired(): void {
    getDb().prepare("DELETE FROM sessions WHERE expires_at < ? OR revoked_at IS NOT NULL").run(new Date(Date.now() - 7 * 86400000).toISOString());
  },
};
...
export type StoredBooking = BookingItem & { ownerId: string; tenantId?: string; propertyCity?: string; propertySlug?: string };
export const bookings = new Collection<StoredBooking>({
  table: "bookings",
  columns: (b) => ({ owner_id: b.ownerId, tenant_id: b.tenantId || null, property_id: b.propertyId, room_id: b.roomId || null, bed_id: b.bedId || null, status: b.bookingStatus }),
});
...
export const customers = new Collection<StoredCustomer>({
  table: "customers",
  columns: (c) => ({ owner_id: c.ownerId, tenant_id: c.tenantId || null, property_id: c.propertyId, status: c.tenantStatus }),
});
...
export const employees = new Collection<StoredEmployee>({
  table: "employees",
  columns: (e) => ({ owner_id: e.ownerId, user_id: e.userId || null, email: e.email.toLowerCase(), role_id: e.roleId, status: e.status }),
});
...
export const payments = new Collection<PaymentRecord>({
  table: "payments",
  columns: (p) => ({ owner_id: p.ownerId, tenant_id: p.tenantId || null, booking_id: p.bookingId || null, customer_id: p.customerId || null, status: p.status, amount: p.amount, idempotency_key: p.idempotencyKey || null }),
});

export type StoredDocument = TenantDocument & { ownerId?: string; tenantId?: string; propertyId?: string; customerId?: string };
export const documents = new Collection<StoredDocument>({
  table: "documents",
  columns: (d) => ({ owner_id: d.ownerId || null, tenant_id: d.tenantId || null, property_id: d.propertyId || null, customer_id: d.customerId || null, status: d.status }),
});

export type StoredTicket = TenantSupportTicket & { tenantId: string };
export const supportTickets = new Collection<StoredTicket>({
  table: "support_tickets",
  columns: (t) => ({ tenant_id: t.tenantId, status: t.status }),
});
...
export const auditEvents = {
  insert(e: AuditEvent): void {
    getDb()
      .prepare(`INSERT INTO audit_events (id, type, actor_id, actor_role, owner_id, aggregate_type, aggregate_id, correlation_id, payload, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(e.id, e.type, e.actorId || null, e.actorRole || null, e.ownerId || null, e.aggregateType || null, e.aggregateId || null, e.correlationId || null, JSON.stringify(e.payload), e.createdAt);
  },
  list(filter: { ownerId?: string; type?: string; limit?: number } = {}): AuditEvent[] {
    const clauses: string[] = [];
    const params: (string | number)[] = [];
    if (filter.ownerId) { clauses.push("owner_id = ?"); params.push(filter.ownerId); }
    if (filter.type) { clauses.push("type LIKE ?"); params.push(`%${filter.type}%`); }
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    const rows = getDb().prepare(`SELECT * FROM audit_events ${where} ORDER BY created_at DESC LIMIT ${Math.floor(filter.limit || 100)}`).all(...params) as Array<Record<string, string>>;
    return rows.map((r) => ({
      id: r.id, type: r.type, actorId: r.actor_id || undefined, actorRole: r.actor_role || undefined, ownerId: r.owner_id || undefined,
      aggregateType: r.aggregate_type || undefined, aggregateId: r.aggregate_id || undefined, correlationId: r.correlation_id || undefined,
...
export const wishlist = {
  list(userId: string): string[] {
    return (getDb().prepare("SELECT property_id FROM wishlist WHERE user_id = ? ORDER BY created_at DESC").all(userId) as Array<{ property_id: string }>).map((r) => r.property_id);
  },
  add(userId: string, propertyId: string): void {
    getDb().prepare("INSERT OR IGNORE INTO wishlist (user_id, property_id, created_at) VALUES (?, ?, ?)").run(userId, propertyId, nowIso());
  },
