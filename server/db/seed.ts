...
import { users, properties, leads, bookings, visitors, customers, crmActivity, roles, employees, rbacAudit, payments, documents, supportTickets, notifications, wishlist } from "./repositories.js";
...
import { getPropertyBySlug as getLegacyPropertyBySlug, detailedPropertyToOwnerPropertyListing } from "../../src/data/propertyDetailsHelper";
import { INITIAL_LEADS_SEED, INITIAL_BOOKINGS_SEED, INITIAL_VISITORS_SEED, INITIAL_CUSTOMERS_SEED, INITIAL_AUDIT_LOGS_SEED } from "../../src/data/crmSeedData";
...
import { DEFAULT_LIVING_PREFERENCES, DEFAULT_NOTIFICATION_SETTINGS, DEFAULT_PRIVACY_SETTINGS } from "../../src/lib/domain/defaults";
...
function seedDemo(): void {
  ...
  // ---- Accounts ------------------------------------------------------------------------------
  users.insert({
    id: ownerId, email: DEMO_ACCOUNTS.owner.email, passwordHash: password, role: "owner", ownerId: null, fullName: DEMO_ACCOUNTS.owner.name,
    status: "active", authProvider: "email",
    data: { phone: "+91 98765 43210", city: "Hyderabad", dob: "1994-08-20", gender: "Male", occupation: "Working Professional", collegeOrCompany: "NestIn Living Spaces", bio: "Founder and Managing Partner at NestIn Living Properties.", language: "English, Telugu, Hindi", notificationSettings: DEFAULT_NOTIFICATION_SETTINGS, privacySettings: DEFAULT_PRIVACY_SETTINGS },
  });
  users.insert({
    id: DEMO_ACCOUNTS.tenant.id, email: DEMO_ACCOUNTS.tenant.email, passwordHash: password, role: "tenant", ownerId: null, fullName: DEMO_ACCOUNTS.tenant.name,
    status: "active", authProvider: "email",
    data: { phone: "+91 98450 12345", avatar: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=300&q=80", city: "Hyderabad", dob: "1999-05-14", gender: "Female", occupation: "Working Professional", collegeOrCompany: "Cognizant Technology Solutions", bio: "Software Engineer relocating to Hitec City. Looking for a verified coliving space with good WiFi.", language: "English (India)", livingPreferences: DEFAULT_LIVING_PREFERENCES, notificationSettings: DEFAULT_NOTIFICATION_SETTINGS, privacySettings: DEFAULT_PRIVACY_SETTINGS },
  });
  ...
  const marketplace = users.insert({
    id: DEMO_ACCOUNTS.marketplace.id, email: DEMO_ACCOUNTS.marketplace.email, passwordHash: password, role: "owner", ownerId: null, fullName: DEMO_ACCOUNTS.marketplace.name,
    status: "active", authProvider: "email", data: { city: "Bengaluru" },
  });
  for (const listing of GENERATED_PROPERTIES_DATA) {
    ...
    properties.insert({ ...converted, id: listing.id, ownerId: marketplace.id, ownerName: marketplace.fullName, ownerEmail: marketplace.email, status: "published" });
  }
  ...
  for (const emp of INITIAL_EMPLOYEES) {
    ...
    if (emp.roleId === "role-owner") {
      ...
      const account = users.insert({
        ...
        email, passwordHash: password, role: "employee", ownerId, fullName: emp.name, status: "active", authProvider: "email",
  ...
  const PROPERTY_REMAP: Record<string, string> = { "prop-ist-gachibowli": "prop-sea-breeze", "prop-stanza-koramangala": "prop-draft-hitech" };
  ...
  for (const lead of INITIAL_LEADS_SEED) leads.insert(remapProperty({ ...lead, email: rewriteEmail(lead.email), ownerId }));
  for (const visit of INITIAL_VISITORS_SEED) visitors.insert(remapProperty({ ...visit, email: rewriteEmail(visit.email), ownerId }));
  ...
  for (const customer of INITIAL_CUSTOMERS_SEED) {
    ...
    const mapped = remapProperty({ ...customer, email, ownerId, tenantId: email === DEMO_ACCOUNTS.tenant.email ? tenantId : undefined });
    ...
    if (slot) {
      ...
      if (mapped.tenantStatus === "Active" || mapped.tenantStatus === "Upcoming") properties.replace(setBedStatus(slot.prop, slot.room.id, slot.bed.id, true, mapped.fullName));
    }
    customers.insert(mapped);
  }
  for (const booking of INITIAL_BOOKINGS_SEED) {
    const tenantEmail = rewriteEmail(booking.tenantEmail)!;
    const mapped = remapProperty({ ...booking, tenantEmail, ownerId, tenantId: tenantEmail === DEMO_ACCOUNTS.tenant.email ? tenantId : undefined });
    ...
    if (customer) {
      ...
      if (slot) {
        ...
        mapped.bedId = slot.bed.id;
        mapped.bedNumber = slot.bed.bedNumber;
        if (mapped.bookingStatus === "Confirmed") properties.replace(setBedStatus(slot.prop, slot.room.id, slot.bed.id, true, mapped.tenantName));
      }
    }
    bookings.insert({ ...mapped, propertyCity: prop?.location?.city, propertySlug: prop?.slug, propertyImage: mapped.propertyImage || prop?.coverImage });
  }
  ...
  for (const pay of INITIAL_TENANT_PAYMENTS) {
    payments.insert({
      id: pay.id, ownerId, tenantId, bookingId: pay.bookingId, propertyName: pay.pgName, tenantName: DEMO_ACCOUNTS.tenant.name,
      amount: pay.amount, currency: "INR", type: pay.type, method: pay.paymentMethod, status: pay.status, gateway: "simulated",
      invoiceNumber: pay.invoiceNumber, transactionId: pay.transactionId, month: pay.month, date: pay.date, createdAt: new Date(pay.date).toISOString(),
    });
  }
  for (const id of ["prop-banyan-premium", "prop-sea-breeze"]) if (properties.exists(id)) wishlist.add(tenantId, id);

  notifications.insert({ id: newId("ntf"), userId: ownerId, title: "Welcome to your NestIn Owner Portal", message: "Your demo workspace is ready: 3 properties, a live CRM pipeline and a staff team have been provisioned.", type: "system", timestamp: new Date().toLocaleString("en-GB"), isRead: false, linkTo: "/owner/dashboard" });
  notifications.insert({ id: newId("ntf"), userId: tenantId, title: "Welcome to NestIn 🏠", message: "Your booking at Nestin Grand Luxury Coliving is confirmed. Move-in date is 1st Sept 2026.", type: "booking", timestamp: new Date().toLocaleString("en-GB"), isRead: false, linkTo: "/my-bookings" });
}
