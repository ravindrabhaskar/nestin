...
export function str(value: unknown, field: string, opts: { required?: boolean; max?: number; min?: number } = {}): string {
...
export function num(value: unknown, field: string, opts: { min?: number; max?: number; required?: boolean; integer?: boolean } = {}): number {
...
export function assertDocumentSize(doc: unknown, maxBytes = 512 * 1024): void {
  ...
  if (size > maxBytes) throw badRequest(`Payload too large (${Math.round(size / 1024)} KB, max ${Math.round(maxBytes / 1024)} KB)`);
}
