...
export function upsertRole(
  ...
  const duplicate = roles
    ...
    .find((r) => r.name.toLowerCase() === name.toLowerCase() && r.id !== existing?.id);
  ...
  const role: Role & { ownerId: string } = {
    ...
    id:
      existing?.id ||
      (isSafeId(id) && !roles.exists(id)
        ? id
        : isSafeId(body.id) && !roles.exists(body.id as string)
          ? (body.id as string)
          : newId('role')),
    ownerId,
    name,
    description: v.optionalStr(body.description ?? existing?.description, 'Description', 500) || '',
    authorityLevel: v.oneOf(
      body.authorityLevel ?? existing?.authorityLevel,
      AUTHORITY_LEVELS,
      'Authority level',
      'medium'
    ),
  ...
  roles.upsert(role);
  events.publish(
    existing ? 'RoleUpdated' : 'RoleCreated',
    'Role',
    role.id,
    { name },
...
export function upsertEmployee(
  ...
  if (emailOwner && emailOwner.id !== existing?.userId) {
    if (emailOwner.role !== 'employee' || emailOwner.ownerId !== ownerId)
      throw conflict('This email address already belongs to another account.');
  }
  ...
  const employee: StoredEmployee = {
    ...
    id:
      existing?.id ||
      (isSafeId(id) && !employees.exists(id)
        ? id
        : isSafeId(body.id) && !employees.exists(body.id as string)
          ? (body.id as string)
          : newId('emp')),
    ownerId,
    name,
    ...
    roleName: role.name,
    status,
    assignedProperties: v
      .arr(body.assignedProperties ?? existing?.assignedProperties ?? ['all'], 'Assigned properties', 200)
      .map((p) => String(p).slice(0, 80)),
    propertyAccessScope: v.oneOf(
      body.propertyAccessScope ?? existing?.propertyAccessScope,
      ['all', 'selected', 'assigned_records'] as const,
      'Access scope',
      'all'
    ),
  ...
  Collection.transaction(() => {
    ...
    if (!account) {
      ...
      users.update(account.id, {
        ...
        fullName: name,
        ownerId,
        data: { ...account.data, employeeId: employee.id, phone: phone || account.data.phone },
      });
  ...
  events.publish(
    existing ? 'EmployeeUpdated' : 'EmployeeCreated',
    'Employee',
    employee.id,
    { email, roleId, status },
    { ...ctx, actorId: actor.id, actorRole: actor.role, ownerId }
