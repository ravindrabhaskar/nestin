...
import { sendEmail, emailTemplate } from '../lib/messaging.js';
...
export function toPublicUser(user: UserRecord, currentSessionId?: string, includeSessions = false): PublicUser {
  ...
  const { employeeId: _ignored, emailVerified, ...profile } = user.data;
  ...
  return {
    ...
    ...profile,
    emailVerified: emailVerified === true || user.authProvider === 'google' || user.authProvider === 'system',
...
function describeClient(req: Request): { device: string; browser: string; ip: string; location: string } {
  ...
  return {
    ...
    browser,
    ip,
    location: ip === '::1' || ip.startsWith('127.') ? 'Local network' : 'Unknown location',
  };
}
...
export function register(body: Record<string, unknown>, req: Request): AuthResult {
  ...
  events.publish(
    ...
    { actorId: user.id, actorRole: user.role, ownerId: role === 'owner' ? user.id : undefined }
  );
...
export function login(body: Record<string, unknown>, req: Request): AuthResult {
  ...
  events.publish(
    ...
    user.id,
    { role: user.role },
    { actorId: user.id, actorRole: user.role, ownerId: user.role === 'owner' ? user.id : user.ownerId || undefined }
  );
  return issueSession(user, req);
}
...
function assertEmployeeActive(user: UserRecord): void {
  ...
  if (!employee || employee.status !== 'active')
    throw forbidden('Your staff account is inactive. Please contact your property owner.');
}
...
export async function loginWithGoogle(body: Record<string, unknown>, req: Request): Promise<AuthResult> {
  if (!config.google.clientId) {
    throw new HttpError(
      ...
      'Google Sign-In is not configured on this server. Set GOOGLE_CLIENT_ID and VITE_GOOGLE_CLIENT_ID.'
    );
  }
  ...
  if (!user) {
    ...
    user = users.insert({
      ...
      status: 'active',
      authProvider: 'google',
      data: { avatar: info.picture, emailVerified: true },
    });
    events.publish(
      'UserRegistered',
      'User',
    ...
    if (user.role === 'super_admin') throw forbidden('Super administrators must sign in through the admin console.');
    if (user.role === 'employee') assertEmployeeActive(user);
    if (!user.data.avatar && info.picture)
      user = users.update(user.id, { data: { ...user.data, avatar: info.picture } }) || user;
  }
  events.publish(
    'UserLoggedIn',
    'User',
    user.id,
    { role: user.role, provider: 'google' },
    { actorId: user.id, actorRole: user.role }
  );
  return issueSession(user, req);
}
...
export function updateProfile(userId: string, sessionId: string, body: Record<string, unknown>): PublicUser {
  ...
  const fullName =
    body.fullName !== undefined
      ? v.str(body.fullName, 'Full name', { max: 120, min: 2 })
      : body.name !== undefined
        ? v.str(body.name, 'Full name', { max: 120, min: 2 })
        : user.fullName;
  ...
  events.publish(
    ...
    { actorId: userId, actorRole: user.role }
  );
...
// ---------------------------------------------------------------------------------------------
// Password reset & email verification (single-use, hashed, expiring tokens delivered by email)
// ---------------------------------------------------------------------------------------------

const RESET_TTL_MS = 60 * 60 * 1000;
const VERIFY_TTL_MS = 24 * 60 * 60 * 1000;

function issueToken(userId: string, kind: 'reset' | 'verify', ttlMs: number): string {
  authTokens.invalidateAll(userId, kind);
...
/** Always responds the same way so the endpoint cannot be used to enumerate accounts. */
export async function requestPasswordReset(body: Record<string, unknown>): Promise<void> {
  const email = v.email(body.email);
  const user = users.findByEmail(email);
  ...
  if (!user.passwordHash) {
    await sendEmail({
      to: user.email,
      subject: 'Your NestIn sign-in method',
      html: emailTemplate(
        'No password to reset',
        [
          `Hi ${user.fullName},`,
          "Your NestIn account signs in with Google, so there is no password to reset. Use 'Continue with Google' on the sign-in screen.",
        ],
        { label: 'Open NestIn', url: config.appUrl }
      ),
    });
    return;
  }
  ...
  await sendEmail({
    to: user.email,
    subject: 'Reset your NestIn password',
    html: emailTemplate(
      'Reset your password',
      [
        `Hi ${user.fullName},`,
        'We received a request to reset the password for your NestIn account. The link below is valid for 1 hour and can be used once.',
        'If you did not request this, you can safely ignore this email.',
      ],
      { label: 'Choose a new password', url }
    ),
  });
  events.publish('PasswordResetRequested', 'User', user.id, {}, { actorId: user.id, actorRole: user.role });
}

export function resetPassword(body: Record<string, unknown>): void {
  const raw = v.str(body.token, 'Token', { max: 200 });
  const passwordError = validatePasswordStrength(body.newPassword);
  if (passwordError) throw badRequest(passwordError);
  const token = authTokens.findValid('reset', hashToken(raw));
  if (!token)
    throw new HttpError(400, 'INVALID_TOKEN', 'This reset link is invalid or has expired. Please request a new one.');
  const user = users.findById(token.userId);
  if (!user) throw new HttpError(400, 'INVALID_TOKEN', 'This reset link is no longer valid.');
  users.update(user.id, { passwordHash: hashPassword(body.newPassword as string) });
  authTokens.consume(token.id);
  sessions.revokeAllForUser(user.id);
  events.publish('PasswordReset', 'User', user.id, {}, { actorId: user.id, actorRole: user.role });
}

export async function sendVerificationEmail(userId: string): Promise<void> {
  const user = users.findById(userId);
  if (!user || user.data.emailVerified) return;
  const raw = issueToken(user.id, 'verify', VERIFY_TTL_MS);
  const url = `${config.appUrl}/verify-email?token=${raw}`;
  await sendEmail({
    to: user.email,
    subject: 'Verify your NestIn email',
    html: emailTemplate(
      'Confirm your email address',
      [
        `Hi ${user.fullName},`,
        'Please confirm this email address to secure your NestIn account and receive booking updates. The link is valid for 24 hours.',
      ],
...
export function verifyEmail(body: Record<string, unknown>): PublicUser {
  const raw = v.str(body.token, 'Token', { max: 200 });
  const token = authTokens.findValid('verify', hashToken(raw));
  if (!token) throw new HttpError(400, 'INVALID_TOKEN', 'This verification link is invalid or has expired.');
  const user = users.findById(token.userId);
  if (!user) throw new HttpError(400, 'INVALID_TOKEN', 'This verification link is no longer valid.');
  const updated = users.update(user.id, { data: { ...user.data, emailVerified: true } })!;
  authTokens.consume(token.id);
  events.publish('EmailVerified', 'User', user.id, {}, { actorId: user.id, actorRole: user.role });
